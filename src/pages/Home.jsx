    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-screen w-full flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-6 lg:px-12 text-center flex flex-col items-center pt-20">
          ...
          >
            <span className="text-accent-primary font-button tracking-[0.2em] text-sm md:text-base mb-4 block">{t('25 TAHUN BERKARYA')}</span>
            ...
            <p className="text-lg md:text-xl max-w-3xl mx-auto font-body text-supporting leading-relaxed mb-10 drop-shadow-md">
              {t('Kisah selama 25 tahun. Dari panggung SMA Pasundan 3 Bandung menuju panggung yang lebih luas bagi seni, budaya, dan generasi mendatang.')}
            </p>
          </motion.div>
      ...
      <section className="py-20 bg-primary border-t border-white/5 relative z-20">
        <div className="container mx-auto px-6 lg:px-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 divide-x divide-white/10">
            <StatCard icon={<Heart className="w-8 h-8 text-accent-primary mb-4" />} number="25+" label={t('Tahun Berkarya')} delay={0.1} />
            <StatCard icon={<Users className="w-8 h-8 text-accent-primary mb-4" />} number="500+" label={t('Alumni')} delay={0.2} />
            <StatCard icon={<Ticket className="w-8 h-8 text-accent-primary mb-4" />} number="100+" label={t('Pementasan')} delay={0.3} />
            <StatCard icon={<Award className="w-8 h-8 text-accent-primary mb-4" />} number="20+" label={t('Prestasi')} delay={0.4} />
          </div>
        </div>
      </section>
    </div>
